...
public class ComputerEquipmentMovingandImprovingBlock extends IgdamodModElements.ModElement {
	...
	public static class CustomBlock extends FallingBlock {
		...
		public CustomBlock() {
			super(Block.Properties.create(Material.IRON).sound(SoundType.METAL).hardnessAndResistance(1.7f, 10f).setLightLevel(s -> 0).harvestLevel(2)
					.harvestTool(ToolType.PICKAXE).setRequiresTool());
			this.setDefaultState(this.stateContainer.getBaseState().with(FACING, Direction.NORTH));
			setRegistryName("computer_equipment_movingand_improving");
		}
			...
			if (world.getRedstonePowerFromNeighbors(new BlockPos(x, y, z)) > 0) {
				{
					...
					ComputerEquipmentMovingandImprovingRedstoneOnProcedure.executeProcedure($_dependencies);
				}
			} else {
			}
		}
